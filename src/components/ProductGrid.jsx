...
function ProductCard({ product }) {
  ...
  return (
    <article className="group">
      <div className="relative overflow-hidden rounded-xl bg-sage-50">
        {product.badge && (
          <span className="absolute left-4 top-4 z-10 rounded-full bg-warm-white px-3 py-1 text-[11px] font-semibold uppercase tracking-wider text-sage-700 shadow-sm">
            {product.badge}
          </span>
        )}
        ...
        <a
          ...
          className="absolute bottom-4 left-4 right-4 flex translate-y-2 items-center justify-center gap-2 rounded-full bg-[#25D366] py-3 text-sm font-semibold text-white opacity-0 transition-all group-hover:translate-y-0 group-hover:opacity-100 hover:bg-[#1fb855]"
        >
      ...
      <div className="mt-4">
        ...
        <h3 className="mt-1 font-display text-lg font-medium leading-snug text-charcoal">
          {product.name}
        </h3>
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="mt-3 inline-flex items-center gap-1.5 text-sm font-semibold text-[#128C7E] transition-colors hover:text-[#075E54] sm:hidden"
        >
          Order on WhatsApp →
        </a>
      </div>
